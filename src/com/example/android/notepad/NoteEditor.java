...
public class NoteEditor extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent.hasExtra(Intent.EXTRA_TEXT)) {
			...
			String text = intent.getExtras().getCharSequence(Intent.EXTRA_TEXT)
					.toString();
	...
	protected void onResume() {
		...
		if (mCursor != null) {
			...
			// If we already have text, leave it as is
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (mState == STATE_EDIT) {
			menu.add(0, SAVE_ID, 0, R.string.menu_save).setShortcut('0', 's')
	...
	/**
	 * Take care of saving a note. Uses Filepicker.io
	 */
	private final void saveNote() {
		File outputDir = NoteEditor.this.getCacheDir(); // context being the
														// Activity pointer
		File outputFile;
		try {
			outputFile = File.createTempFile("fp_temp", "txt", outputDir);
			FileWriter writer = new FileWriter(outputFile);
			writer.append(mText.getText());
			writer.close();
			Uri uri = Uri.fromFile(outputFile);

			startActivityForResult(new Intent(FilePicker.SAVE_CONTENT, uri,
